...
- Payloads run in a new POSIX process group. The worker also tracks observed descendants by PID/start ticks, including children creating another session. TERM is followed by KILL after the configured grace. The grace is cleanup time and can extend beyond the job deadline; it is not additional training budget. A child that daemonizes/reparents before being observed can escape this polling backend; such jobs require a future cgroup/container backend.
